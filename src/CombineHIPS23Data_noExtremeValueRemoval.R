...
lnkYieldData <- lnkYieldData %>%
  ...
  mutate(plotNumber = as.integer(plotNumber), 
         ...
         plotLength = 17.5) %>%
  mutate(
    # yieldPerAcre = case_when(earsMissingFromCenterRows > 0 ~ NA, .default = yieldPerAcre),
...
df2 <- df2 %>%
  rowwise() %>%
  mutate(
    plantDensity = case_when(
      # plantDensity < 20000 | plantDensity > 50000 ~ NA,
...
hybridHIPS23 <- mutate(hybridHIPS23, year = '2023') %>%
  ...
  mutate(plantingDate = as.character(plantingDate), 
         ...
         harvestDate = as.character(harvestDate) %>%
           str_split_i(' ', 1))
hybrids22 <- read.csv('outData/HIPS_2022_V3.7_HYBRIDS_noExtremeValueRemoval.csv') %>%
  mutate(year = '2022') %>%
  rowwise() %>%
...
write.csv(hybridHIPS, 'outData/HIPS_HYBRIDS_2022_AND_2023_V2.3_noExtremeValueRemoval.csv',  quote = FALSE, sep = ',', row.names = FALSE, col.names = TRUE)



compDatasets <- full_join(hybrids122, hybridsExtremeValues, join_by(year, location, nitrogenTreatment, irrigationProvided, genotype, block, plotNumber))
extra <- compDatasets %>%
  filter((is.na(plantDensity.extreme) & !is.na(plantDensity.clean)) |
           (is.na(earHeight.extreme) & !is.na(earHeight.clean)) |
           (is.na(flagLeafHeight.extreme) & !is.na(flagLeafHeight.clean))) %>%
  select(year, location, nitrogenTreatment, genotype,
         plantDensity.clean, plantDensity.extreme, 
         earHeight.clean, earHeight.extreme, 
         flagLeafHeight.clean, flagLeafHeight.extreme)
