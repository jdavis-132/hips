...
top10PercentOverallPerformancePlot <- ggplot(top10PercentOverallPerformance, aes(top10Percent, yield_mtha, fill = top10Percent)) +
  ...
  labs(x = '', y = 'Yield (MT/ha)', fill = 'Overall Hybrid Rank') +
  theme_minimal() + 
  theme(axis.text.x = element_text(color = 'black', size = 9),
        axis.text.y = element_text(color = 'black', size = 9),
...
sigCrossovers <- read_csv('../../../Downloads/significantCrossovers.txt')

heatmap <- plotInteractionImportanceGrid(trait = 'yieldPerAcre', 
                                         traitLabel = 'Yield (MT/ha)', 
...
mostPlastic10PercentPerformance <- ggplot(hybrids, aes(mostPlastic10Percent, yield_mtha, fill = mostPlastic10Percent)) + 
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), color = 'white') +
  ...
  scale_fill_manual(values = viridis_pal()(4)[1:2], 
                    labels = c('Lower 90%', 'Upper 10%')) + 
  labs(x = '', y = 'Yield (MT/ha)', fill = 'Plasticity Rank') +
  theme_minimal() + 
  theme(axis.text.x = element_text(color = 'black', size = 9),
        axis.text.y = element_text(color = 'black', size = 9),
        strip.text = element_text(color = 'black', size = 9),
...
percentEnvMeanPlotPlasticityVersusRelativePerformance <- ggplot(percentEnvMean.pl, aes(meanRelativePerformance, b, color = meanParentAge)) + 
  ...
  labs(x = 'Mean Hybrid Percent of Environment Mean', y = 'Linear Plasticity \n All Environments', 
       color = 'Mean Parent Release Year') +
  theme_minimal() +
  theme(axis.text.x = element_text(color = 'black', size = 9, hjust = 0.5),
        axis.text.y = element_text(color = 'black', size = 9),
        text = element_text(color = 'black', size = 9, hjust = 0.5),
...
percentEnvMeanPlotPlasticityVersusRelativePerformance <- ggplot(percentEnvMean.pl, aes(meanRelativePerformance, b, color = meanParentAge)) + 
  ...
  # scale_y_continuous(limits = c(-5.6, 1.6)) +
  labs(x = 'Mean Hybrid Percent of Environment Mean', y = 'Linear Plasticity \n All Environments', 
       color = str_wrap('Mean Parent Release Year', 1)) +
  theme_minimal() +
  theme(axis.text.x = element_text(color = 'black', size = 9, hjust = 0.5),
        axis.text.y = element_text(color = 'black', size = 9),
        text = element_text(color = 'black', size = 9, hjust = 0.5),
...
percentEnvMeanPlotPlasticityVersusMeanYield <- ggplot(percentEnvMean.pl, aes(convertUSYieldsToMTPerHa(meanYield), b, color = meanParentAge)) + 
  geom_point() + 
  # geom_hline(yintercept = mean(percentEnvMean.pl$b)) +
  scale_color_viridis(direction = -1) + 
  ...
  # scale_y_continuous(breaks = c(50, 100, 150, 200), 
  #                    labels = c('50%', '100%', '150%', '200%')) +
  labs(x = 'Mean Hybrid Yield (MT/ha)', y = 'Linear Plasticity \n All Environments', 
       color = str_wrap('Mean Parent Release Year', 1)) +
  theme_minimal() +
  theme(axis.text.x = element_text(color = 'black', size = 9, hjust = 0.5),
        axis.text.y = element_text(color = 'black', size = 9),
...
tradeoffMeanYieldWorseEnvs <- ggplot(worseEnvironmentPercentMean.pl, aes(convertUSYieldsToMTPerHa(hybridMeanWorseEnvironments), b, color = meanParentAge)) +
  geom_point() + 
  # geom_hline(yintercept = mean(worseEnvironmentPercentMean.pl$b, na.rm = TRUE)) +
  scale_color_viridis(direction = -1) + 
  labs(x = 'Mean Hybrid Yield (MT/ha)', y = 'Linear Plasticity \n 14 Worst Environments', 
...
tradeoffMeanYieldBetterEnvs <- ggplot(betterEnvironmentPercentMean.pl, aes(convertUSYieldsToMTPerHa(hybridMeanBetterEnvironments), b, color = meanParentAge)) +
  geom_point() + 
  # geom_hline(yintercept = mean(betterEnvironmentPercentMean.pl$b, na.rm = TRUE)) +
  scale_color_viridis(direction = -1) + 
  labs(x = 'Mean Hybrid Yield (MT/ha)', y = 'Linear Plasticity \n 14 Best Environments', 
       color = str_wrap('Mean Parent Release Year', 1), title = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(color = 'black', size = 9, hjust = 0.5),
        axis.text.y = element_text(color = 'black', size = 9),
...
gca_vp <- gca_vp %>%
  rowwise() %>%
  mutate(grp = case_when(grp=='earParent' ~ 'Parental GCA',
                         grp=='pollenParent' ~ 'Parental GCA',
                         .default = grp) %>%
           factor(levels = c('Parental GCA', 'Residual'))) %>%
  group_by(grp, responseVar, label) %>%
  summarise(pctVar = sum(pctVar, na.rm = TRUE))

gcaImportanceOrder <- gca_vp %>%
  filter(grp=='Parental GCA') %>%
  arrange(desc(pctVar))
...
interactionImportanceConceptualScores <- sigCrossovers %>%
  ...
  # select(c(genotype1, genotype2, contains('yieldPerAcreScore'), contains('yieldPerAcreSigDiff'))) %>%
  # select(c(genotype1, genotype2, contains('10'), contains('yieldPerAcreSigDiff'))) %>%
  # filter(if_any(contains('yieldPerAcreScore.'), ~ .x == 1)) %>%
  # select(c(genotype1, genotype2, where(~ is.numeric(.) && max(., na.rm = TRUE) > 1)))
  filter(genotype1=='LH145 X LH162' & genotype2=='B84 X 3IIH6') #%>%
  # select(contains('yieldPerAcreScore.E11'))
  select(genotype1, genotype2, yieldPerAcreSigDiff.E10, yieldPerAcreSigDiff.E26, yieldPerAcreSigDiff.E25, 
         yieldPerAcreSigDiff.E32, yieldPerAcreRank.E10.G1, yieldPerAcreRank.E10.G2, yieldPerAcreRank.E26.G1,
         yieldPerAcreRank.E26.G2, yieldPerAcreRank.E25.G1, yieldPerAcreRank.E25.G2, yieldPerAcreRank.E32.G1, 
         yieldPerAcreRank.E32.G2, `yieldPerAcreScore.E10-26`, `yieldPerAcreScore.E10-25`, 
         `yieldPerAcreScore.E10-32`)
View(interactionImportanceConceptualScores)

hybrids <- hybrids %>% 
  rowwise() %>% 
  mutate(yield_mtha = convertUSYieldsToMTPerHa(yieldPerAcre.sp))
scoreZeroData <- filter(hybrids, (genotype %in% c('LH145 X LH162', 'B84 X 3IIH6')) & 
                      (environment %in% c('2022 Missouri Valley Medium', '2023 Ames Medium'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', '2023 Ames Medium')))
scoreZeroEnvs <- filter(hybrids, (environment %in% c('2022 Missouri Valley Medium', '2023 Ames Medium'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', '2023 Ames Medium')))

scoreZeroPlot <- ggplot(scoreZeroData, aes(environment, yield_mtha, 
                                           color = genotype, 
                                           group = genotype)) + 
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE) +
  geom_boxplot(aes(environment, yield_mtha, group = environment), 
              data = scoreZeroEnvs, 
              color = 'black', fill = NA) + 
  annotate('text', x = '2022 Missouri Valley Medium', y = 12.56, label = '*', size = 9, color = 'black') +
  scale_color_manual(values = viridis_pal()(4)[1:2]) +
  scale_x_discrete(breaks = c('2022 Missouri Valley Medium', '2023 Ames Medium'),
                   labels = c(str_wrap('2022 Missouri Valley Medium', 16), str_wrap('2023 Ames Medium', 16))) +
  scale_y_continuous(limits = c(0.6, 12.56)) +
  labs(x = 'Environment', y = 'Yield (MT/ha)', color = 'Genotype', 
...
scoreOneData <- filter(hybrids, (genotype %in% c('LH145 X LH162', 'B84 X 3IIH6')) & 
                          (environment %in% c('2022 Missouri Valley Medium', '2022 North Platte FI High'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', 
                                                      '2022 North Platte FI High')))
scoreOneEnvs <- filter(hybrids, (environment %in% c('2022 Missouri Valley Medium', '2022 North Platte FI High'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', '2022 North Platte FI High')))

scoreOnePlot <- ggplot(scoreOneData, aes(environment, yield_mtha, color = genotype, group = genotype)) + 
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE) +
  geom_boxplot(aes(environment, yield_mtha, group = environment), 
              data = scoreOneEnvs, 
  ...
  annotate('text', x = '2022 Missouri Valley Medium', y = 12.56, label = '*', size = 9, color = 'black') +
  scale_color_manual(values = viridis_pal()(4)[1:2]) +
  scale_x_discrete(breaks = c('2022 Missouri Valley Medium', '2022 North Platte FI High'),
                   labels = c(str_wrap('2022 Missouri Valley Medium', 16), 
                              str_wrap('2022 North Platte FI High', 16))) +
  scale_y_continuous(limits = c(0.6, 12.56)) +
  labs(x = 'Environment', y = 'Yield (MT/ha)', color = 'Genotype', 
...
scoreTwoData <- filter(hybrids, (genotype %in% c('LH145 X LH162', 'B84 X 3IIH6')) & 
                         (environment %in% c('2022 Missouri Valley Medium', '2023 Crawfordsville High'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', '2023 Crawfordsville High')))
scoreTwoEnvs <- filter(hybrids, (environment %in% c('2022 Missouri Valley Medium', '2023 Crawfordsville High'))) %>%
  mutate(environment = factor(environment, levels = c('2022 Missouri Valley Medium', '2023 Crawfordsville High')))

scoreTwoPlot <- ggplot(scoreTwoData, aes(environment, yield_mtha, color = genotype, group = genotype)) + 
  geom_point() + 
  geom_smooth(method = 'lm', se = FALSE) +
  geom_boxplot(aes(environment, yield_mtha, group = environment), 
              data = scoreTwoEnvs, 
              color = 'black', fill = NA) + 
  annotate('text', x = '2022 Missouri Valley Medium', y = 12.56, label = '*', size = 9, color = 'black') +
  annotate('text', x = '2023 Crawfordsville High', y = 12.56, label = '*', size = 9, color = 'black') +
  scale_color_manual(values = viridis_pal()(4)[1:2]) +
  scale_x_discrete(breaks = c('2022 Missouri Valley Medium', '2023 Crawfordsville High'),
                   labels = c(str_wrap('2022 Missouri Valley Medium', 16), str_wrap('2023 Crawfordsville High', 16))) +
  scale_y_continuous(limits = c(0.6, 12.56)) +
  labs(x = 'Environment', y = 'Yield (MT/ha)', color = 'Genotype', 
